...
void DrawStatsMenu()
{
    ...
    case STATPAGE_STATS:
    {
        ...
        HudMessage("Regen Sphere: %d Sec", (int)(5.0 + ((fixed)Player.RegenerationTotal / 5.0)));
...
void UpgradeTurret(int Index)
{
    ...
    if (Player.Turret.Upgrade[Index] < TurretUpgradeData[Index].MaxLevel)
    {
        ...
        else if (CheckInventory("DRPGTurretPart") >= TurretUpgradeCost(Index))
        {
            ...
            {
                TurretDespawn();
                Player.Turret.RefitTimer += Player.Turret.Upgrade[Index] * (2 - Player.Turret.Upgrade[TU_HARDWARE_SPECS]);
            }
            else // Build
            {
                Player.Turret.Health = 100;
                Player.Turret.RefitTimer += 10;
