...
NamedScript Type_OPEN void GlobalInit()
{
    ...
    {
        // Version Info
        Log("\CnDoom RPG SE [EXPERIMENTAL-STARFOX] (GDCC) (Compiled on %S) loaded!", __DATE__);
...
NamedScript OptionalArgs(1) void DynamicLootGenerator(str Actor, int MaxItems)
{
    ...
    if (MaxItems == 0)
    {
        fixed SkillLevelDivision = (10 - (SKILL_LEVEL / 5));
        if (SkillLevelDivision < 1)
            SkillLevelDivision = 1;
        MaxItems = Random(LevelNum / 5, LevelNum) + ((AveragePlayerLuck() + AveragePlayerLevel()) / SkillLevelDivision);
